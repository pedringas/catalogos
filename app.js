...
const state = {
    ...
    creationDate: getTodayLocalISO(), // YYYY-MM-DD (fecha local), hoy por defecto
    observations: '',
...
function getFooterText() {
    // state.creationDate viene como YYYY-MM-DD; lo mostramos como "Creado DD/MM/YYYY".
    // Partimos el string a mano (no new Date()) para evitar corrimientos de día por huso horario.
    if (!state.creationDate) return '';
    const [y, m, d] = state.creationDate.split('-');
    if (!y || !m || !d) return '';
    return `Creado ${d}/${m}/${y}`;
}
...
// Fecha de creación: reflejar el valor por defecto (hoy) en el input y escuchar cambios
if (ui.creationDate) {
    ui.creationDate.value = state.creationDate;
    ui.creationDate.addEventListener('change', (e) => state.creationDate = e.target.value);
}
